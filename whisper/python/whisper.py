...
def mel_filters(n_mels):
    ...
    filters_path = MODEL_DIR / "mel_80_filters.txt"
...
def init_model(model_path, target=None, device_id=None):
    if model_path.endswith(".rknn"):
        if RKNNLite is not None:
            ...
            print('Neither RKNNLite nor RKNN python package is available.')
            exit(1)
...
if __name__ == '__main__':
    ...
    # Set inputs
    if args.task == "en":
        vocab_path = MODEL_DIR / "vocab_en.txt"
        task_code = 50259
    elif args.task == "zh":
        vocab_path = MODEL_DIR / "vocab_zh.txt"
        task_code = 50260
    else:
